...
define('EMAILMGR_CURRENT_VERSION', "1.3.12");
...
if (!class_exists('cpanelapi', false)) {
...
if (function_exists('is_multisite') && is_multisite()) {
	add_action('remove_user_from_blog', 'emailmanager_remove_user'); 
} else {
...
function _emailmanager_activate() {
	...
	$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_matchemail", "option_value"=>0));
	$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_mailinglists", "option_value"=>""));
	$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_mailinglistuser", "option_value"=>"both"));
	$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_ssl", "option_value"=>1));
}
...
function _emailmanager_deactivate() {
	...
	$wpdb->delete($wpdb->options, array("option_name"=>"emailmgr_matchemail"));
	$wpdb->delete($wpdb->options, array("option_name"=>"emailmgr_mailinglists"));
	$wpdb->delete($wpdb->options, array("option_name"=>"emailmgr_mailinglistuser"));
	$wpdb->delete($wpdb->options, array("option_name"=>"emailmgr_ssl"));
}
...
function emailmanager_create_email($user_id, $firstname, $lastname, $username, $email, $customformat = null, $customalias = null, $customquota = null, $custompass = null) {
	//Set config options
	$autogenerate = emailmanager_config_options("emailmgr_autogen");
	...
	$matchemail = emailmanager_config_options("emailmgr_matchemail");
	$emailalias = emailmanager_assign_alias($firstname, $lastname, $username, $email, $customformat, $customalias);
	$mailinglists = emailmanager_config_options("emailmgr_mailinglists");
	$mailinglistuser = emailmanager_config_options("emailmgr_mailinglistuser");
	...
	$newemail = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "createemail", $optional);

	if ($newemail["status"] && !empty($user_id)) {
		global $wpdb;
		...
		if ($matchemail) {
			wp_update_user(array('ID' => $user_id, 'user_email' => $options['email']));
		}
		if (!EMAILMGR_STD_MODE && $autogenerate) {
			if ($matchemail) {
				$wpemail = $options['email'];
			} else {
				$wpemail = $options['regemail'];
			}
			if ($mailinglists != "") {
				$mailinglists = explode(",", $mailinglists);
				foreach ($mailinglists as $list) {
					if ($mailinglistuser == "cpanel") {
						emailmanager_pro_mailinglist_add(str_replace("@", "_", $list), $emaildomain, $options['email'], $cpanelpass);
					} elseif ($mailinglistuser == "wordpress") {
						if (!empty($wpemail)) {
							emailmanager_pro_mailinglist_add(str_replace("@", "_", $list), $emaildomain, $wpemail, $cpanelpass);
						}
					} elseif ($mailinglistuser == "both") {
						emailmanager_pro_mailinglist_add(str_replace("@", "_", $list), $emaildomain, $options['email'], $cpanelpass);
						if (!$matchemail && !empty($options['regemail'])) {
							emailmanager_pro_mailinglist_add(str_replace("@", "_", $list), $emaildomain, $options['regemail'], $cpanelpass);
						}
					}
				}
			}
		...
		return '<div class="error"><p>' . __('The following error occurred', 'cpanel-e-mail-manager') . ': ' . (!empty($newemail["response"]) ? $newemail["response"] : sprintf(__('An unknown error occurred while trying to create the e-mail account for %s', 'cpanel-e-mail-manager'), $emailalias)) . '</p>
		</div><br>';
	}
}
...
function emailmanager_remove_email($emailalias) {
	...
	if (EMAILMGR_STD_MODE == 0) {
		...
		$lists = emailmanager_pro_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "listlists", $optional);
		if (!empty($lists["response"][0]->list)) {
			if (!empty($lists["response"][0])) {
...
function emailmanager_upgrade() {
	...
	if (version_compare($currentversion, "1.2.7", "<")) {
		/* Upgrade to 1.2.7 */
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_mailinglist_cleanup", "option_value"=>0));
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_pw_uppercase", "option_value"=>1));
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_pw_lowercase", "option_value"=>1));
	...
	if (version_compare($currentversion, "1.3.2", "<")) {
		/* Upgrade to 1.3.2 */
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_matchemail", "option_value"=>0));
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_mailinglists", "option_value"=>""));
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_mailinglistuser", "option_value"=>"both"));
	...
	if (version_compare($currentversion, "1.3.11", "<")) {
		/* Upgrade to 1.3.11 */
		$wpdb->insert($wpdb->options, array("option_name"=>"emailmgr_ssl", "option_value"=>1));
...
function emailmanager_output() {
	...
	if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "") {
		if (function_exists('is_multisite') && is_multisite() && $_GET["mode"] != "config") {
			...
			$domains = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "listemaildomains");
			$domainfound = 0;
			foreach ($domains["response"] as $domain) {
			...
			if ($domainfound == 0) {
				...
				<p>&nbsp;</p>
				<p align="center" style="font-size:14px;"><a href="users.php?page=emailmanager&mode=config">' . __('Update Configuration Settings', 'cpanel-e-mail-manager') . '</a></p>
                ';
				exit;
			}
		}
	...
	if (isset( $_GET['mode'] ) && !empty( $_GET['mode'] )) {
		...
		if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "" && $emaildomain != "") {
			$accountinfo = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "diskusage");
			if ($accountinfo["response"]->max != "") {
				$output .= emailmanager_output_list();
			} else {
				$output .= emailmanager_output_config($emaildomain);
			}
...
function emailmanager_output_config($mudomain = null) {
	...
	if (!empty($_POST)) {
		if (isset($_GET["action"])) {
			if ($_GET["action"] == "createdns") {
				...
				$cpanelpass = emailmanager_config_options("emailmgr_cpanelpass");
				$cpanelserver = emailmanager_config_options("emailmgr_cpanelip");
				$ssl = emailmanager_config_options("emailmgr_ssl");

				$optional['domain'] = $_POST['emaildomain'];
				$result = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "addparkeddomain", $optional);
				if ($result["status"]) {
					...
					$error = emailmanager_resultLogging(__('Park Domain', 'cpanel-e-mail-manager'), (!isset($result["response"]) ? sprintf(__('An unspecified error has occurred while trying to add a parked domain for %s.', 'cpanel-e-mail-manager'), $_POST['emaildomain']) : $result["response"]));
				...
				$ssl = (isset($_POST['ssl']) ? $_POST['ssl'] : 0);
			...
			$matchemail = (isset($_POST['matchemail']) ? $_POST['matchemail'] : 0);
			$mailinglists = (isset($_POST['mailinglists']) ? implode(",", $_POST['mailinglists']) : "");
			$mailinglistuser = (isset($_POST['mailinglistuser']) ? $_POST['mailinglistuser'] : "both");
			...
			emailmanager_update_config("emailmgr_ssl", $ssl);
			if ($cpaneluser != "") {
				emailmanager_update_config("emailmgr_cpaneluser", $cpaneluser);
			}
			if ($cpanelpass != "") {
			...
			emailmanager_update_config("emailmgr_matchemail", $matchemail);
			emailmanager_update_config("emailmgr_mailinglists", $mailinglists);
			emailmanager_update_config("emailmgr_mailinglistuser", $mailinglistuser);
		}
	...
	$cpanelpass = emailmanager_config_options("emailmgr_cpanelpass");
	$cpanelserver = emailmanager_config_options("emailmgr_cpanelip");
	$ssl = (emailmanager_config_options("emailmgr_ssl") ? " checked" : "");
	$emailmsg = emailmanager_config_options("emailmgr_emailmsg");
	...
	$matchemail = (emailmanager_config_options("emailmgr_matchemail") ? " checked" : "");
	$mailinglists = emailmanager_config_options("emailmgr_mailinglists");
	$mailinglistuser = emailmanager_config_options("emailmgr_mailinglistuser");
	...
	if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "") {
		...
		if ($accountinfo["response"]->max != "") {
			...
			if (document.getElementById("autogenerate").checked) {
			  ...
			  document.getElementById("matchemail").disabled = false;
			  document.getElementById("mailinglists").disabled = false;
			  document.getElementById("mailinglistuser").disabled = false;
			  ...
			  document.getElementById("matchemail").disabled = true;
			  document.getElementById("mailinglists").disabled = true;
			  document.getElementById("mailinglistuser").disabled = true;
			...
			<tr><td align="left" colspan="3"><p style="font-weight: bold; text-decoration:underline;">' . __('Mailing Lists', 'cpanel-e-mail-manager') . '</p></td></tr>
			<tr><td align="left" colspan="3">&nbsp;</td></tr>
			<tr><td align="left" width="35%">
			<label for="mailinglists" style="vertical-align: top;">' . __('Automatically Assign Mailing Lists', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><select name="mailinglists[]" id="mailinglists" size="6" multiple>';
			if ($mailinglists != "") {
				$mailinglists = explode(",", $mailinglists);
			} else {
				$mailinglists = array();
			...
			$optional['domain'] = $emaildomain;
			if (!EMAILMGR_STD_MODE) {
				$lists = emailmanager_pro_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "listlists", $optional);
				if (!empty($lists["response"][0]->list)) {
					if (!empty($lists["response"][0])) {
						foreach ($lists["response"] as $list) {
							if (in_array($list->list, $mailinglists)) {
								$output .= '<option value="' . $list->list . '" selected>' . $list->list . '</option>';
							} else {
								$output .= '<option value="' . $list->list . '">' . $list->list . '</option>';
							}
						}
					} else {
			...
			$output .= '</select></td><td align="left" width="45%"><label for="mailinglists" style="vertical-align: top;"><em>' . __('Select the mailing lists that new WordPress users should be automatically assigned to.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>
			<tr><td align="left" width="35%"><label for="mailinglistuser" style="vertical-align: top;">' . __('Mailing List E-Mail Selection', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><select name="mailinglistuser" id="mailinglistuser">';
			if ($mailinglistuser == "cpanel") {
				$output .= '<option value="cpanel" selected>cPanel</option>';
			} else {
				$output .= '<option value="cpanel">cPanel</option>';
			}
			if ($mailinglistuser == "wordpress") {
				$output .= '<option value="wordpress" selected>WordPress</option>';
			} else {
				$output .= '<option value="wordpress">WordPress</option>';
			}
			if ($mailinglistuser == "both") {
				$output .= '<option value="both" selected>cPanel + WordPress</option>';
			} else {
				$output .= '<option value="both">cPanel + WordPress</option>';
			}
			$output .= '</select></td><td align="left" width="45%"><label for="mailinglistuser" style="vertical-align: top;"><em>' . __('Choose the e-mail address to be added to the mailing lists selected above.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			if (function_exists('is_multisite') && is_multisite()) {
				$output .= '<tr><td align="left" width="35%"><label for="mailinglist_cleanup" style="vertical-align: top;">' . __('Mailing List Cleanup', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="checkbox" name="mailinglist_cleanup" id="mailinglist_cleanup"' . $mailinglist_cleanup . '></td><td align="left" width="45%"><label for="mailinglist_cleanup" style="vertical-align: top;"><em>' . __('Automatically remove WordPress user\'s e-mail addresses from associated mailing lists when users are removed from this blog.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			} else {
				$output .= '<tr><td align="left" width="35%"><label for="mailinglist_cleanup" style="vertical-align: top;">' . __('Mailing List Cleanup', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="checkbox" name="mailinglist_cleanup" id="mailinglist_cleanup"' . $mailinglist_cleanup . '></td><td align="left" width="45%"><label for="mailinglist_cleanup" style="vertical-align: top;"><em>' . __('Automatically remove WordPress user\'s e-mail addresses from associated mailing lists when a WordPress user is deleted.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			}
			$output .= '<tr><td align="left" colspan="3"><hr></td></tr>
			<tr><td align="left" colspan="3"><p style="font-weight: bold; text-decoration:underline;">' . __('Remove E-Mail Accounts', 'cpanel-e-mail-manager') . '</p></td></tr>
			<tr><td align="left" colspan="3"><label for="removeusers" style="vertical-align: top;font-weight:bold;"><em><font color="red"><strong><u>' . __('WARNING', 'cpanel-e-mail-manager') . ':</u> ' . __('This action is irreversible and will delete all data from the user\'s e-mail account.', 'cpanel-e-mail-manager') . '</strong></font></em></label></td></tr>
			<tr><td align="left" colspan="3">&nbsp;</td></tr>';
			if (function_exists('is_multisite') && is_multisite()) {
				$output .= '<tr><td align="left" width="35%"><label for="removeuser" style="vertical-align: top;">' . __('Delete E-Mail Account When Removing Network Users', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="checkbox" name="removeuser" id="removeuser"' . $removeuser . '></td><td align="left" width="45%"><label for="removeuser" style="vertical-align: top;"><em>' . __('If enabled, existing cPanel e-mail accounts will be deleted when users are removed from this blog.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			} else {
				$output .= '<tr><td align="left" width="35%"><label for="removeuser" style="vertical-align: top;">' . __('Delete E-Mail Account When WordPress User is Deleted', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="checkbox" name="removeuser" id="removeuser"' . $removeuser . '></td><td align="left" width="45%"><label for="removeuser" style="vertical-align: top;"><em>' . __('Automatically delete associated cPanel e-mail account when a WordPress user is deleted.', 'cpanel-e-mail-manager') . '</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			}
			$output .= '<tr><td align="left" colspan="3"><hr></td></tr>
			...
			if (!is_multisite()) {
				...
				$accountinfo = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "emailcount");
				$output .= $accountinfo["response"]->count . ' / ' . $accountinfo["response"]->max;
			...
			if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "") {
				$domains = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "listemaildomains");
				foreach ($domains["response"] as $domain) {
                    $domainval = (substr($domain->domain, 0, 4) == "www." ? substr($domain->domain, 4) : $domain->domain);
					if (function_exists('is_multisite') && is_multisite()) {
						if ($domainval == $emaildomain) {
							$output .= '<option value="'. $domainval .'" selected>'. $domainval .'</option>';
						}
					} else {
						if ($domainval == $emaildomain) {
							$output .= '<option value="'. $domainval .'" selected>'. $domainval .'</option>';
						} else {
							$output .= '<option value="'. $domainval .'">'. $domainval .'</option>';
						}
					}
				}
			}
			...
			if (EMAILMGR_CPANEL_HIDE == 1 || EMAILMGR_CPANEL_GLOBAL == 1) {
				...
				<tr><td align="left" width="35%"><label for="cpanelserver" style="vertical-align: top;">' . __('cPanel Server IP', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="text" name="cpanelserver" id="cpanelserver" value="' . $cpanelserver . '"></td><td align="left" width="45%"><label for="cpanelserver" style="vertical-align: top;"><em>' . __('Enter', 'cpanel-e-mail-manager') . ' \'localhost\' ' . __('or', 'cpanel-e-mail-manager') . ' \'127.0.0.1\' ' . __('if logging into cPanel on the same server as this WordPress install', 'cpanel-e-mail-manager') . '.</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>
				<tr><td align="left" width="35%"><label for="ssl" style="vertical-align: top;">' . __('Use SSL', 'cpanel-e-mail-manager') . ':</label></td><td align="left" width="20%"><input type="checkbox" name="ssl" id="ssl"' . $ssl . '></td><td align="left" width="45%"><label for="ssl" style="vertical-align: top;"><em>' . __('Check this box if your cPanel server uses a secure connection (e.g. https)', 'cpanel-e-mail-manager') . '.</em></label></td></tr><tr><td align="left" colspan="3">&nbsp;</td></tr>';
			}
			$output .= '<tr><td align="left" colspan="3"><hr></td></tr>
			...
			if (EMAILMGR_CPANEL_HIDE == 1 || EMAILMGR_CPANEL_GLOBAL == 1) {
				...
				<p><label for="ssl" style="vertical-align: top; float: left; width: 10em;">' . __('Use SSL', 'cpanel-e-mail-manager') . ':</label> <input type="checkbox" name="ssl" id="ssl"' . $ssl . ' style="float: left;"><label for="ssl" style="vertical-align: top;"><em>' . __('Check this box if your cPanel server uses a secure connection (e.g. https)', 'cpanel-e-mail-manager') . '.</em></label></p>
				<p>&nbsp;</p>
				<p>&nbsp;</p>
...
function emailmanager_output_list() {
	...
	if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "" && $emaildomain != "") {
		...
		$optional['domain'] = $emaildomain;
		$accounts = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "getaccountlist", $optional);
        $accountrslts = array();
        $accountcnt = 0;
        if (!empty($_GET["keyword"])) {
            $keyword = $_GET["keyword"];
    		if (!empty($accounts["response"][0]->user)) {
    			foreach ($accounts["response"] as $account) {
    				if (emailmanager_emailalias_userid($account->user) != "") {
                        if (strpos($keyword, '@') !== false) {
                            ...
                            if (strpos(strtolower($account->login), strtolower($keyword)) !== false || strpos(strtolower($account->user), strtolower($keyword)) !== false || strpos(strtolower($user_info->first_name), strtolower($keyword)) !== false || strpos(strtolower($user_info->last_name), strtolower($keyword)) !== false || strpos(strtolower($user_info->user_login), strtolower($keyword)) !== false) {
                                $accountrslts[$accountcnt] = $account;
                                $accountcnt++;
                            }
                        } else {
        					$user_info = get_userdata(emailmanager_emailalias_userid($account->user));
                            if (strpos(strtolower($account->user), strtolower($keyword)) !== false || strpos(strtolower($user_info->first_name), strtolower($keyword)) !== false || strpos(strtolower($user_info->last_name), strtolower($keyword)) !== false || strpos(strtolower($user_info->user_login), strtolower($keyword)) !== false) {
                                $accountrslts[$accountcnt] = $account;
                                $accountcnt++;
                            }
                        }
                        ...
                        if (strpos($keyword, '@') !== false) {
                            if (strpos(strtolower($account->login), strtolower($keyword)) !== false || strpos(strtolower($account->user), strtolower($keyword)) !== false) {
                                $accountrslts[$accountcnt] = $account;
                                $accountcnt++;
                            }
                        } else {
                            if (strpos(strtolower($account->user), strtolower($keyword)) !== false) {
                                $accountrslts[$accountcnt] = $account;
                                $accountcnt++;
                            }
                        }
		...
		if (!empty($accountrslts)) {
			...
			function emailSearch() {
			  if (document.getElementById("emailkeyword").value != "")
			  {
			    document.email_search.action = location.protocol + "//" + location.hostname + location.pathname + "?page=emailmanager&keyword=" + document.getElementById("emailkeyword").value;
			...
			$output .= '<table style="width:96%; margin-left:auto; margin-right:auto;"><tr><td align="right"><div class="tablenav-pages">
			<form action="users.php?page=emailmanager" method="post" name="email_search" id="email_search" style="display:inline" onsubmit="return emailSearch();">
			<input type="text" name="emailkeyword" id="emailkeyword" style="display:inline;">&nbsp;<input type="submit" name="submit" id="submit" value="' . __('Search', 'cpanel-e-mail-manager') . '" style="display:inline;" class="button button-primary">
			</form>';
            if (!empty($_GET["keyword"])) {
    			$output .= '&nbsp;&nbsp;(<a href="users.php?page=emailmanager">' . __('List All Accounts', 'cpanel-e-mail-manager') . '</a>)';
            }
			$output .= '&nbsp;&nbsp;<span class="displaying-num">(' . $numrecords . ' ' . __('items', 'cpanel-e-mail-manager') . ')&nbsp;</span><span class="pagination-links">';

			if ($currpage != 1) {
				$output .= '<span class="tablenav-pages-navspan" onclick="location.href = \'users.php?page=emailmanager' . (!empty($_GET["keyword"]) ? "&keyword=" . $_GET["keyword"] : "") . '&paged=1\'" style="cursor: pointer;">&laquo;</span>&nbsp;<span class="tablenav-pages-navspan" onclick="location.href = \'users.php?page=emailmanager' . (!empty($_GET["keyword"]) ? "&keyword=" . $_GET["keyword"] : "") . '&paged=' . $prevpage . '\'" style="cursor: pointer;">&lsaquo;</span>&nbsp;';
			}
			...
			if ($currpage != $numpages) {
				$output .= '&nbsp;<span class="tablenav-pages-navspan" onclick="location.href = \'users.php?page=emailmanager' . (!empty($_GET["keyword"]) ? "&keyword=" . $_GET["keyword"] : "") . '&paged=' . $nextpage . '\'" style="cursor: pointer;">&rsaquo;</span>&nbsp;<span class="tablenav-pages-navspan" onclick="location.href = \'users.php?page=emailmanager' . (!empty($_GET["keyword"]) ? "&keyword=" . $_GET["keyword"] : "") . '&paged=' . $numpages . '\'" style="cursor: pointer;">&raquo;</span>';
			}
			...
			$sortarray = array();
			$cnt = 0;
			foreach ($accountrslts as $account) {
				if (emailmanager_emailalias_userid($account->user) != "") {
					$user_info = get_userdata(emailmanager_emailalias_userid($account->user));
			...
			if ($sortarray != "") {
				...
				$output .= '<tr><td colspan="4"><p>&nbsp;</p><p style="text-align: center; font-weight: bold;font-size: 16px;">' . __('There are no accounts to display.', 'cpanel-e-mail-manager') . '</p>
				<p style="text-align: center; font-size: 16px;"><a href="users.php?page=emailmanager&mode=add">' . __('Add a New Account', 'cpanel-e-mail-manager') . '</a>';
		        if (!empty($_GET["keyword"])) {
    				$output .= ' ' . __('or', 'cpanel-e-mail-manager') . ' <a href="users.php?page=emailmanager">' . __('List All Accounts', 'cpanel-e-mail-manager') . '</a>';
		        }
				$output .= '</p><p>&nbsp;</p></td></tr>';
			}
			$output .= '</tbody></table>
			...
			$output .= '<p>&nbsp;</p><p>&nbsp;</p><p style="text-align: center; font-weight: bold;font-size: 16px;">' . __('There are no accounts to display.', 'cpanel-e-mail-manager') . '</p><p style="text-align: center; font-size: 16px;"><a href="users.php?page=emailmanager&mode=add">' . __('Add a New Account', 'cpanel-e-mail-manager') . '</a>';
		    if (!empty($_GET["keyword"])) {
    		    $output .= ' ' . __('or', 'cpanel-e-mail-manager') . ' <a href="users.php?page=emailmanager">' . __('List All Accounts', 'cpanel-e-mail-manager') . '</a>';
	        }
			$output .= '</p>';
...
function emailmanager_output_adduser() {
	...
	if ($cpaneluser != "" && $cpanelpass != "" && $cpanelserver != "" && $emaildomain != "") {
		...
		if ($accountinfo["response"]->max != "") {
			$accountinfo = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "emailcount");
			if ((int)$accountinfo["response"]->count < (int)$accountinfo["response"]->max || $accountinfo["response"]->max == "unlimited") {
...
function emailmanager_output_upgradeplugin() {
	global $wpdb;
	$output = '';
	$output .= '<h2 style="display:inline;">' . __('Upgrade Plugin', 'cpanel-e-mail-manager') . '</h2> - <a href="users.php?page=emailmanager&mode=config">' . __('Configuration Options', 'cpanel-e-mail-manager') . '</a><p>&nbsp;</p><p style="text-align:center; font-weight: bold;font-size: 16px;">' . __('This plugin has been discontinued and the Pro version is no longer available.', 'cpanel-e-mail-manager') . '</p>';
	$output .= '<p style="text-align:center; font-weight: bold;font-size: 16px;">' . __('You may continue using the Standard version of this plugin at your own risk, but support will not be offered for WordPress installations above v5.0.3.', 'cpanel-e-mail-manager') . '</p>';
	return $output;
}
...
function emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, $action, $optional = null) {
	//CPANEL LOGIN
    $usessl = emailmanager_config_options("emailmgr_ssl");
	$cpanelapi = new cpanel_email_manager_api($cpanelserver, $usessl);

	//CREATE CPANEL EMAIL ACCOUNT
	if ($action == "createemail") {
        $settings['username'] = $cpaneluser;
        $settings['password'] = $cpanelpass;
        $options['user'] = strtolower($settings["username"]);
        $options['domain'] = $optional['email_domain'];
        $options['email'] = $optional['email_user'];
        $options['password'] = $optional['email_pass'];
        $options['quota'] = $optional['email_quota'];
        return $cpanelapi->createemailaccount($settings, $options);
	}
	...
	if ($action == "deleteemail") {
        ...
        $options['domain'] = $optional['email_domain'];
        $options['email'] = $optional['email_user'];
        return $cpanelapi->removeemailaccount($settings, $options);
	}
	...
	if ($action == "changepw") {
        ...
        return $cpanelapi->changeemailpw($settings, $options);
	}
	...
	if ($action == "addparkeddomain") {
        ...
        $settings['password'] = $cpanelpass;
        $options['user'] = strtolower($settings["username"]);
        $options['domain'] = $optional['domain'];
        return $cpanelapi->createparkeddomain($settings, $options);
	}
	...
	if ($action == "listemaildomains") {
        ...
        $options['user'] = strtolower($settings["username"]);
        $options['skipmain'] = 0;
        return $cpanelapi->listemaildomains($settings, $options);
	}
	...
	if ($action == "getaccountinfo") {
        ...
        $settings['password'] = $cpanelpass;
        $options['user'] = strtolower($settings["username"]);
        $options['regex'] = $optional['email_user'] . "@";
        return $cpanelapi->getemailaccountdata($settings, $options);
	}
	...
	if ($action == "getaccountlist") {
        ...
        $settings['password'] = $cpanelpass;
        $options['user'] = strtolower($settings["username"]);
        $options['regex'] = '@'.$optional['domain'];
        return $cpanelapi->getemailaccountlist($settings, $options);
	}
	...
	if ($action == "emailcount") {
        ...
        $settings['password'] = $cpanelpass;
        $options['user'] = strtolower($settings["username"]);
        $options['display'] = "emailaccounts";
        return $cpanelapi->listemailaccountstats($settings, $options);
	}
...
function emailmanager_assign_alias($firstname, $lastname, $username, $email = null, $customformat = null, $customalias = null) {
	//Set config options
	$autogenerate = (emailmanager_config_options("emailmgr_autogen") ? " checked" : "");
	$wordpressgroups = emailmanager_config_options("emailmgr_groups");
	$emaildomain = emailmanager_config_options("emailmgr_domain");
	...
	if ($alias != "") {
		...
		$aliascheck = emailmanager_cpanelAPI($cpaneluser, $cpanelpass, $cpanelserver, "getaccountinfo", $optional);
        if (!empty($aliascheck["response"]->user)) {	
    		foreach ($aliascheck["response"] as $singlealias) {
    			$alias_tmp = $alias . "." . $cnt;
    			$optional['email_domain'] = $emaildomain;
    			$optional['email_user'] = $alias_tmp;
    			$aliascheck = $singlealias->user;
    			$cnt++;
    		}
        }
	}
...
class Cpanelcipher
{
	...
	function __construct($textkey)
	{
    	...
    	{
		if ($input != "") {
            $pass = base64_encode($input) . ":" . $this->securekey;
            $pass = $this->securekey . ":" . base64_encode($pass);
            $pass = $this->securekey . ":" . base64_encode($pass) . ":" . $this->securekey;
	...
	{
		if ($input != "") {
            $pass = base64_decode($input);
            $pass = str_replace($this->securekey . ":", "", str_replace(":" . $this->securekey, "", $pass));
